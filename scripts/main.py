...
def build_model(sess, x, y_, train, labels):
    ...
    for i in range(100): #range(20000):
        (train_a, _, label_a, _) = cross_validation.train_test_split(train, train_labels, train_size=100)
        if i % 100 == 0:
            train_accuracy = sess.run(accuracy, feed_dict={x:train_a, y_: label_a, keep_prob: 1.0})
            print "step %d, training accuracy %g"%(i, train_accuracy)
        sess.run(train_step, feed_dict={x: train_a, y_: label_a, keep_prob: 0.5})
...
model = build_model(session, x, y_, train, train_labels)
# y = session.run(model, feed_dict={x: test, keep_prob: 1.0})
# print(y)
